...
mt_t *init_mt(void* public_keys, const unsigned int number_of_msg, void (*fill_leaf_nodes)())
{
    ...
    (*fill_leaf_nodes)(mt, public_keys); // Fill leaf nodes
...
int mt_verify_public_with_aux(uint8_t* public_key, uint8_t* sent_hash, uint8_t* aux, unsigned int index_of_msg, unsigned int num_of_leaf_nodes)
{
    unsigned char msg_hash_512[crypto_hash_sha512_BYTES];
    crypto_hash_sha512_state state;
    int i = 0, mt_height, next_power_of_two;
    uint8_t* addr = aux;
